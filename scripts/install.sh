...
BIN_DIR="${AGENTBOARD_INSTALL_BIN_DIR:-$HOME/.local/bin}"
...
# board.sqlite3 is owned entirely by the running program (paths.ts'
# dataDirectory); the installer only ever prints its path.
board_database_path() {
  local base="$HOME/.local/share"
  if [[ -n "${XDG_DATA_HOME:-}" && "$XDG_DATA_HOME" == /* ]]; then
    base="$XDG_DATA_HOME"
  fi
  printf '%s\n' "$base/agentboard/board.sqlite3"
}
...
With no option, installs agentboard. Installation runs Bun's frozen dependency
install, links ~/.local/bin/agentboard to this checkout, and writes the
deployed Git SHA to ~/.local/state/agentboard/deployed-sha.

The board database at $(board_database_path) is never created, migrated, or
deleted by this script.

Set AGENTBOARD_INSTALL_BIN_DIR and AGENTBOARD_INSTALL_STATE_DIR to use other
...
validate_managed_checkout() {
  ...
  validate_safe_file "$source" "agentboard source command"
  [[ -x "$source" ]] || die "Refusing non-executable agentboard source command: $source"
  sha="$(checkout_head "$root")" || die "Refusing agentboard source outside an exact Git checkout: $root"
  origin="$(git -C "$root" remote get-url origin 2>/dev/null)" || die "Refusing agentboard source without an origin: $root"
  [[ "$(normalized_origin "$origin")" == "$EXPECTED_ORIGIN" ]] || die "Refusing agentboard source with foreign origin: $root"
  MANAGED_ROOT="$root"
  MANAGED_SHA="$sha"
}
...
install_agentboard() {
  ...
  TMP_PATH="$BIN_DIR/.agentboard-link.$$.$RANDOM"
...
uninstall_agentboard() {
  ...
  if (( removed )); then
    echo "Removed agentboard installation"
  else
    echo "Agentboard is not installed"
  fi
  echo "The board database at $(board_database_path) was left untouched"
}
